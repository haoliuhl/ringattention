    ...
    for i, ckpt_path in enumerate(ckpt_paths):
        ...
        for k, v in checkpoint.items():
            if k.startswith("model."):
                k = k[6:]
            ckpt[k] = v
    print(f"Start converting from hf weight...")
    jax_weights = {
        "transformer": {
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="convert from hf format script")
