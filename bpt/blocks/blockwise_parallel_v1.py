...
def blockwise_compute(cell, q_inputs, kv_inputs, position_ids, num_heads, bias, deterministic, dropout_rng, attn_pdrop, causal_mask, query_chunk_size, key_chunk_size, dtype, policy, precision, prevent_cse):
    ...
    def _query_chunk_attention(cell, _, args):
        ...
        init_carry = Carry(
            jnp.zeros((batch, query_chunk_size, num_heads, dim_per_head), dtype=q_inputs.dtype),
            jnp.zeros((batch, query_chunk_size, num_heads, dim_per_head), dtype=q_inputs.dtype),
            (-jnp.inf) * jnp.ones((batch, query_chunk_size, num_heads, 1), dtype=q_inputs.dtype),
        )
